...
export class TypeOrmLoggerContainer implements TypeOrmLogger {
  static ForConnection(connectionName: string, options: TypeOrmLoggerOptions) {
    const logger = new MyLogger(`TypeORM[${connectionName}]`)
  ...
  logQueryError(
    ...
    if (
      ...
      this._logger.error(`error: ` + (isString(error) ? error : error.message))
    }
  }
  ...
  private isToLog(logType: 'query' | 'schema' | 'error' | 'warn' | 'info' | 'log' | 'migration'): boolean {
    return this._options === 'all'
      || (this._options instanceof Array && this._options.indexOf(logType) !== -1)
  }
}
